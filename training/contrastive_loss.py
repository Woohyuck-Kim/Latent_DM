...
def get_similarity_matrix(outputs, chunk=2, multi_gpu=False, help_ood=None):
    '''
    ...
    if help_ood == None:
        ...
        sim_matrix = torch.mm(outputs, torch.cat((outputs, help_ood)).t())  # (B', d), (d, B'+H) -> (B', B'+H)

    return sim_matrix


def NT_xent(sim_matrix, temperature=0.5, chunk=2, eps=1e-8, help=False):
    '''
        Compute NT_xent loss
        - sim_matrix: (B', B') tensor for B' = B * chunk (first 2B are pos samples)
        - If help: sim_matrix is a (B', B'+H) tensor
    '''
    ...
    if help:
        sim_matrix = sim_matrix[:, :B * chunk]
        help_matrix = sim_matrix[:, B * chunk:]

    eye = torch.eye(B * chunk).to(device)  # (B', B')
    sim_matrix = torch.exp(sim_matrix / temperature) * (1 - eye)  # remove diagonal
    if help:
        help_matrix = torch.exp(help_matrix / temperature)

    denom = torch.sum(sim_matrix, dim=1, keepdim=True)
    if help:
        denom = denom + torch.sum(help_matrix, dim=1, keepdim=True)
