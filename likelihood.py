...
def get_likelihood_fn(sde, inverse_scaler, hutchinson_type='Rademacher',
                      rtol=1e-3, atol=1e-4, method='RK45', eps=1e-5):
  ...
  def likelihood_fn(model, data, mode="bpd"):
    ...
    with torch.no_grad():
      ...
      nfe = solution.nfev
      zp = solution.y[:, -1]
      #solution, infodict = integrate.odeint(ode_func, init, np.array([eps, sde.T]), full_output=True, rtol=rtol, atol=atol, mxstep=1000, tfirst=True)
     # nfe = infodict['nfe']
     # zp = np.transpose(solution)[:, -1]
      z = mutils.from_flattened_numpy(zp[:-shape[0]], shape).to(data.device).type(torch.float32)

     # with open("sample.pkl", "wb") as f:
       # pickle.dump(mutils.to_flattened_numpy(data, shape).to(data.device).type(torch.float32), f)
     # with open("latent_sample.pkl", "wb") as g:
       # pickle.dump(z, g)
...
def get_LLR_fn(sde, inverse_scaler, hutchinson_type='Rademacher',
                      rtol=1e-5, atol=1e-5, method='RK45', eps=1e-5):

  def LLR_fn(model, background_model, data):

    likelihood_fn = get_likelihood_fn(sde, inverse_scaler)
    LLR = likelihood_fn(model, data, mode="LLR") - likelihood_fn(background_model, data, mode="LLR")
