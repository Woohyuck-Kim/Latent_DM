...
from datasets_1 import get_dataset, get_superclass_list, get_subclass_dataset
from utils_1.utils import load_checkpoint
...
model = torch.nn.DataParallel(model).to(torch.device('cuda:0') if torch.cuda.is_available() else torch.device('cpu'))
model.train()
...
optimizer = torch.optim.Adam(score_model.parameters(), lr=2e-4, betas=(0.9, 0.999), eps=1e-8,
                             weight_decay=0)
score_model_state = dict(optimizer=optimizer, model=score_model, ema=ema, step=0)
score_model_state = restore_checkpoint('./checkpoints/checkpoint_ResMLP_new_10.pth', score_model_state,
                           device=torch.device('cuda:0') if torch.cuda.is_available() else torch.device('cpu'))
ema.copy_to(score_model.parameters())
# Build data iterators

std = torch.load('std_new.pt', map_location=torch.device('cpu')).cuda()
mean = torch.load('mean_new.pt', map_location=torch.device('cpu')).cuda()
